...
/**
 * @brief 自定义BatchNorm2d层，tensorrt中只能添加Scale层，这里主要是将BatchNorm2d层的参数转换为对应的Scale参数
 * 
 * @param network: 网络定义指针用于整体网络结构的创建
 * @param weightMap: 网络结构对应的权重参数文件保存为map形式
 * @param input: 该卷积块的输入Tensor
 * @param lname: 该层所在层级的名称
 * @param eps: 精度参数，用在分母中防止出现分母为0的情况
 * 
 * @return 添加好该卷积块的网络结构定义参数
*/
...
/**
 * @brief 使用Mish激活函数的卷积块，卷积层中只使用了权重忽略了偏置，该卷积块中主要实现了：卷积层+batchNorm层+自定义的Mish激活函数层
 * 
 * @param network: 网络定义指针用于整体网络结构的创建
 * @param weightMap: 网络结构对应的权重参数文件保存为map形式
 * @param input: 该卷积块的输入Tensor
 * @param outch: 该卷积块的输出通道数
 * @param ksize: 卷积块的卷积核尺寸
 * @param s: 卷积层的stride大小
 * @param p: 卷积层的padding大小
 * @param linx: 该卷积块在网络结构中所处的层次编号
 * 
 * @return 添加好该卷积块的网络结构定义参数
*/
ILayer* convBnMish(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int outch, int ksize, int s, int p, int linx) {
    ...
    /*TensorRT中用来添加自定义层的流程，参见TensorRT-Developer-Guide4 P33*/
    //使用函数getPluginRegistry()从全局的TensorRT Plugin Registry中获取自定义的Mish_TRT层
...
/**
 * @brief 使用LeakyRELU激活函数的卷积块，卷积层中只使用了权重忽略了偏置，该卷积块中主要实现了：卷积层+batchNorm层+LeakyRELU激活层
 * 
 * @param network: 网络定义指针用于整体网络结构的创建
 * @param weightMap: 网络结构对应的权重参数文件保存为map形式
 * @param input: 该卷积块的输入Tensor
...
//读入给定文件夹下所有的文件名称并存入file_names中返回
...
int main(int argc, char** argv) {
    ...
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        // 读入保存的yolov4.engine并反序列化生成推理引擎执行推理
    ...
    if(!cap.isOpened())
    {
        std::cout << "Open camera failed, please check your device!" << std::endl;
        return -1;
    }
    ...
    cv::namedWindow("YOLOv4 Object Detection Window", cv::WINDOW_NORMAL);
    ...
    while (true)
    {
        ...
        auto outPut = img;
        //英伟达TensorRT闭源无法通过断点调试解决其中的部分问题，之前在这部分出现段错误主要是因为data数组中的数据没有从0开始，从而导致赋值时超出了原有的界限产生错误
        for (int b = 0; b < fcount; b++)
        {
            cv::Mat pr_img = preprocess_img(img);
            //std::cout << pr_img.cols << " " << pr_img.rows << std::endl;
            //将读入的三通道图片存入input data中用于推理
        ...
        for (size_t j = 0; j < res.size(); j++) {
            ...
            //}
            //std::cout << std::endl;
            cv::Rect r = get_rect(outPut, res[j].bbox);
            cv::rectangle(outPut, r, cv::Scalar(0x27, 0xC1, 0x36), 2);
            cv::putText(outPut, std::to_string((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
        }
        //在显示界面上添加FPS值
